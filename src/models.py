...
class HeaderTypes(Enum):
    ...
    HISTORY_INCREASE_BUFFER = (
        3  # Increased buffer size of packets, which invalidates old replays
    )
    HISTORY_SAVE_ENGINE_VERSION = (
        4  # Now saving engine net version + InternalProtocolVersion
    )
    HISTORY_EXTRA_VERSION = (
        5  # We now save engine/game protocol version, checksum, and changelist
    )
    HISTORY_MULTIPLE_LEVELS = (
        6  # Replays support seamless travel between levels
    )
    HISTORY_MULTIPLE_LEVELS_TIME_CHANGES = (
        7  # Save out the time that level changes happen
    )
    HISTORY_DELETED_STARTUP_ACTORS = (
        8  # Save DeletedNetStartupActors inside checkpoints
    )
    ...
    HISTORY_SAVE_FULL_ENGINE_VERSION = (
        11  # Now saving the entire FEngineVersion including branch name
    )
    HISTORY_HEADER_GUID = 12  # Save guid to demo header
    HISTORY_CHARACTER_MOVEMENT = (
        13  # Change to using replicated movement and not interpolation
    )
    HISTORY_CHARACTER_MOVEMENT_NOINTERP = (
        14  # No longer recording interpolated movement samples
    )
